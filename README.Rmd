...
If a subgroup column is a factor, `table()` and `plot()` respect its factor
level order. This is useful when you want to keep a meaningful predefined order,
such as `"Low"`, `"Medium"`, and `"High"`, instead of ordering categories by
their contributions.

For example, consider representing the same results not as a numerical range but as departure types: `"Early"`, `"On-time"`, and `"Delayed"`.
...
When `departure_type` is of character type, plots or tables are sorted by contribution order.
On the other hand, when it's of factor type, they are sorted by the order of levels.
...
You can change the order of the factor levels to display them in any desired sequence.
